              ...
              val updatedCyaModel: EmploymentCYAModel = {
                accommodationRelocation match {
                  case Some(accommodationRelocationModel) if yesNo =>
                    cya.copy(employmentBenefits = benefits.map(_.copy(accommodationRelocationModel = Some(accommodationRelocationModel.copy(accommodationRelocationQuestion = Some(true))))))
                  case Some(_) =>
                    cya.copy(employmentBenefits = benefits.map(_.copy(accommodationRelocationModel = Some(AccommodationRelocationModel.clear))))
                  case _ =>
                    cya.copy(employmentBenefits = benefits.map(_.copy(accommodationRelocationModel = Some(AccommodationRelocationModel(accommodationRelocationQuestion = Some(yesNo))))))
                }
              }
