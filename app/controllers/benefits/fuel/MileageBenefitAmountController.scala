...
class MileageBenefitAmountController @Inject()(authAction: AuthorisedAction,
                                               pageView: MileageBenefitAmountView,
    ...
    inYearAction.notInYear(taxYear) {
      employmentSessionService.getAndHandle(taxYear, employmentId) { (optCya, _) =>
        ...
        redirectService.redirectBasedOnCurrentAnswers(taxYear, employmentId, optCya, EmploymentBenefitsType)(redirects(_, taxYear, employmentId)) { cya =>
          ...
          val form = cyaAmount.fold(formsProvider.mileageAmountForm(request.user.isAgent)) { amount =>
            formsProvider.mileageAmountForm(request.user.isAgent).fill(amount)
          }
    ...
    inYearAction.notInYear(taxYear) {
      ...
      employmentSessionService.getSessionDataAndReturnResult(taxYear, employmentId)(redirectUrl) { cya =>
        redirectService.redirectBasedOnCurrentAnswers(taxYear, employmentId, Some(cya), EmploymentBenefitsType)(redirects(_, taxYear, employmentId)) { cya =>
          ...
          formsProvider.mileageAmountForm(request.user.isAgent).bindFromRequest().fold(
            formWithErrors => Future.successful(BadRequest(pageView(taxYear, formWithErrors, employmentId))),
  ...
  private def handleSuccessForm(taxYear: Int, employmentId: String, employmentUserData: EmploymentUserData, amount: BigDecimal)
    ...
    fuelService.updateMileage(request.user, taxYear, employmentId, employmentUserData, amount).map {
