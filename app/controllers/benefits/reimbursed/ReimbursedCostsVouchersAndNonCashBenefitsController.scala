...
class ReimbursedCostsVouchersAndNonCashBenefitsController @Inject()(authAction: AuthorisedAction,
  ...
  private def handleSuccessForm(taxYear: Int, employmentId: String, employmentUserData: EmploymentUserData, questionValue: Boolean)
    ...
    reimbursedService.updateSectionQuestion(request.user, taxYear, employmentId, employmentUserData, questionValue).map {
