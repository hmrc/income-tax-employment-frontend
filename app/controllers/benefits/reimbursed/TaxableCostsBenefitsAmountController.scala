...
class TaxableCostsBenefitsAmountController @Inject()(authAction: AuthorisedAction,
    ...
    inYearAction.notInYear(taxYear) {
      employmentSessionService.getAndHandle(taxYear, employmentId) { (optCya, _) =>
        redirectService.redirectBasedOnCurrentAnswers(taxYear, employmentId, optCya, EmploymentBenefitsType)(redirects(_, taxYear, employmentId)) { cya =>
          ...
          val form = cyaAmount.fold(formsProvider.taxableCostsAmountForm(request.user.isAgent)) { amount =>
            formsProvider.taxableCostsAmountForm(request.user.isAgent).fill(amount)
          }
  ...
  private def handleSuccessForm(taxYear: Int, employmentId: String, employmentUserData: EmploymentUserData, amount: BigDecimal)
    ...
    reimbursedService.updateTaxableExpenses(request.user, taxYear, employmentId, employmentUserData, amount).map {
  ...
  private def redirects(cya: EmploymentCYAModel, taxYear: Int, employmentId: String): Seq[ConditionalRedirect] = {
    redirectService.taxableExpensesAmountRedirects(cya, taxYear, employmentId)
  }
}
