...
class VouchersBenefitsController @Inject()(implicit val cc: MessagesControllerComponents,
                                           authAction: AuthorisedAction,
                                           inYearAction: InYearAction,
                                           view: VouchersBenefitsView,
                                           appConfig: AppConfig,
  ...
  private def handleSuccessForm(taxYear: Int, employmentId: String, employmentUserData: EmploymentUserData, questionValue: Boolean)
    ...
    reimbursedService.updateVouchersAndCreditCardsQuestion(taxYear, employmentId, employmentUserData, questionValue).map {
  ...
  private def yesNoForm(implicit user: User[_]): Form[Boolean] = YesNoForm.yesNoForm(
    missingInputError = s"benefits.vouchersBenefits.error.${if (user.isAgent) "agent" else "individual"}"
  )
}
