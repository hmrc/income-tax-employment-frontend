...
class EntertainmentBenefitsAmountController @Inject()(authAction: AuthorisedAction,
    ...
    inYearAction.notInYear(taxYear) {
      employmentSessionService.getAndHandle(taxYear, employmentId) { (optCya, _) =>
        ...
        redirectService.redirectBasedOnCurrentAnswers(taxYear, employmentId, optCya,
          EmploymentBenefitsType)(redirectService.entertainmentBenefitsAmountRedirects(_, taxYear, employmentId)) { cya =>
          val cyaAmount = cya.employment.employmentBenefits.flatMap(_.travelEntertainmentModel.flatMap(_.entertaining))
          val form = cyaAmount.fold(formsProvider.entertainmentBenefitsAmountForm(request.user.isAgent)) { amount =>
            formsProvider.entertainmentBenefitsAmountForm(request.user.isAgent).fill(amount)
          }
  ...
  private def handleSuccessForm(taxYear: Int, employmentId: String, employmentUserData: EmploymentUserData, amount: BigDecimal)
    ...
    travelService.updateEntertaining(request.user, taxYear, employmentId, employmentUserData, amount).map {
