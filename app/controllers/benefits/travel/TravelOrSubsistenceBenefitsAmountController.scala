...
import controllers.benefits.travel.routes.IncidentalOvernightCostEmploymentBenefitsController
...
class TravelOrSubsistenceBenefitsAmountController @Inject()(authAction: AuthorisedAction,
    ...
    inYearAction.notInYear(taxYear) {
      employmentSessionService.getAndHandle(taxYear, employmentId) { (optCya, _) =>
        redirectService.redirectBasedOnCurrentAnswers(taxYear, employmentId, optCya, EmploymentBenefitsType)(redirects(_, taxYear, employmentId)) { cya =>
          ...
          val form = cyaAmount.fold(formsProvider.travelOrSubsistenceBenefitsAmountForm(request.user.isAgent)) { amount =>
            formsProvider.travelOrSubsistenceBenefitsAmountForm(request.user.isAgent).fill(amount)
          }
  ...
  private def handleSuccessForm(taxYear: Int, employmentId: String, employmentUserData: EmploymentUserData, amount: BigDecimal)
    ...
    travelService.updateTravelAndSubsistence(request.user, taxYear, employmentId, employmentUserData, amount).map {
