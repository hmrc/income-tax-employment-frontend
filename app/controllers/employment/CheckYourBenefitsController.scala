...
class CheckYourBenefitsController @Inject()(pageView: CheckYourBenefitsView,
              ...
              case Some(EmploymentSourceOrigin(source, isUsingCustomerData)) =>
                source.employmentBenefits.flatMap(_.benefits) match {
                  ...
                  case Some(benefits) =>
                    ...
                    Ok(pageView(taxYear, employmentId, source.employerName, benefits.toBenefitsViewModel(isUsingCustomerData), isUsingCustomerData = false, isInYear = true, showNotification = false))
                }
              case None => redirect
            }
        }
          ...
          (cya, prior) =>
            (cya, prior) match {
              case (_, Some(allEmploymentData)) if allEmploymentData.eoyEmploymentSourceWith(employmentId).exists(!_.employmentSource.employmentDetailsSubmittable) =>
                ...
                Future.successful(Ok(pageView(taxYear, employmentId, sourceOrigin.employmentSource.employerName, benefits.toBenefitsViewModel(sourceOrigin.isCustomerData), sourceOrigin.isCustomerData, isInYear = false, showNotification = true)))
              case (Some(cya), _) =>
                ...
                case Some(EmploymentSourceOrigin(source, isUsingCustomerData)) =>
                  employmentSessionService.createOrUpdateSessionData(request.user, taxYear, employmentId, EmploymentCYAModel(source, isUsingCustomerData),
                    ...
                    benefits match {
                      ...
                      case Some(benefits) =>
                        Ok(pageView(taxYear, employmentId, source.employerName, benefits.toBenefitsViewModel(isUsingCustomerData), isUsingCustomerData, isInYear = false, showNotification = false))
                    }
                  })
                case None =>
  ...
  def submit(taxYear: Int, employmentId: String): Action[AnyContent] = authAction.async {
    implicit request =>
