...
class DeleteOrIgnoreExpensesService @Inject()(deleteOverrideExpensesConnector: DeleteOrIgnoreExpensesConnector,
  ...
  def deleteOrIgnoreExpenses(employmentData: AllEmploymentData, taxYear: Int)(result: Result)
    ...
    val hmrcExpenses: Option[EmploymentExpenses] = employmentData.hmrcExpenses.filter(_.dateIgnored.isEmpty)
    val customerExpenses: Option[EmploymentExpenses] = employmentData.customerExpenses

    (hmrcExpenses, customerExpenses) match {
