...
class ReceiveAnyBenefitsControllerISpec extends IntegrationTest with ViewHelpers with EmploymentDatabaseHelper {
  ...
  def cyaModel(hmrc: Boolean, isBenefitsReceived: Option[Boolean] = None): EmploymentCYAModel = EmploymentCYAModel(EmploymentDetails(
    currentDataIsHmrcHeld = !hmrc, employerName = "test"), BenefitsViewModel(isUsingCustomerData = hmrc, isBenefitsReceived = isBenefitsReceived))
      ...
      s"language is ${welshTest(user.isWelsh)} and request is from an ${agentTest(user.isAgent)}" should {
        ...
        "return Did you receive any benefits question page" when {
          ...
          lazy val result: WSResponse = {
            ...
            dropEmploymentDB()
            insertCyaData(employmentUserData(false, cyaModel(false)), userRequest)
        ...
        "return Did you receive any benefits question page with radio button pre-filled if isBenefits received field true" when {
          ...
          lazy val result: WSResponse = {
            authoriseAgentOrIndividual(user.isAgent)
            dropEmploymentDB()
            insertCyaData(employmentUserData(false, cyaModel(false, Some(true))), userRequest)
        ...
        "redirect to Check your benefits page when there is no cya" when {
          ...
          "redirect to Check Employment Details page" in {
            result.header(HeaderNames.LOCATION) shouldBe Some(CheckYourBenefitsController.show(taxYear, employmentID).url)
          }
        }
  ...
  ".submit" should {
    ...
    userScenarios.foreach { user =>
      ...
      s"language is ${welshTest(user.isWelsh)} and request is from an ${agentTest(user.isAgent)}" should {
        ...
        "return Did you receive receive any employments Page with errors when no radio button is selected" when {
          implicit lazy val result: WSResponse = {
            authoriseAgentOrIndividual(user.isAgent)
            dropEmploymentDB()
            insertCyaData(employmentUserData(false, cyaModel(false)), userRequest)
        ...
        "redirect to Check your benefits page when radio button yes is selected" when {
          lazy val result: WSResponse = {
            dropEmploymentDB()
            authoriseAgentOrIndividual(user.isAgent)
            insertCyaData(employmentUserData(false, cyaModel(false)), userRequest)
          ...
          "redirect to Check Your Benefits page" in {
            ...
            cyamodel.employment.employmentBenefits.isBenefitsReceived shouldBe Some(true)
          }
        }

        "redirect to Check your benefits page when radio button no is selected" when {
          lazy val result: WSResponse = {
            dropEmploymentDB()
            authoriseAgentOrIndividual(user.isAgent)
            insertCyaData(employmentUserData(false, cyaModel(false)), userRequest)
          ...
          "redirect to Check Benefits Details page" in {
            result.header(HeaderNames.LOCATION) shouldBe Some(CheckYourBenefitsController.show(validTaxYear2021, employmentID).url)
            lazy val cyamodel = findCyaData(validTaxYear2021, employmentID, userRequest).get
            cyamodel.employment.employmentBenefits.isBenefitsReceived shouldBe Some(false)
            cyamodel.employment.employmentBenefits shouldBe BenefitsViewModel(isUsingCustomerData = false, isBenefitsReceived = Some(false))
          }
        }
