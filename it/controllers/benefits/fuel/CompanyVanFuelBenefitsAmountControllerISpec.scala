      ...
      s"language is ${welshTest(user.isWelsh)} and request is from an ${agentTest(user.isAgent)}" should {
        "render the company van fuel benefits amount page without pre-filled form" which {
          lazy val result: WSResponse = {
            ...
            urlGet(fullUrl(vanFuelBenefitsAmountUrl(taxYearEOY, employmentId)), welsh = user.isWelsh, headers = Seq(HeaderNames.COOKIE -> playSessionCookies(taxYearEOY)))
          }
          ...
          formPostLinkCheck(vanFuelBenefitsAmountUrl(taxYearEOY, employmentId), continueButtonFormSelector)
      ...
      s"language is ${welshTest(user.isWelsh)} and request is from an ${agentTest(user.isAgent)}" should {
        "should render How much was your company van fuel benefit? page with empty error text when there no input" which {
          implicit lazy val result: WSResponse = {
            ...
            dropEmploymentDB()
            insertCyaData(anEmploymentUserData)
            urlPost(fullUrl(vanFuelBenefitsAmountUrl(taxYearEOY, employmentId)), welsh = user.isWelsh, headers = Seq(HeaderNames.COOKIE -> playSessionCookies(taxYear)), body = Map[String, String]())
          }
        ...
        "should render How much was your company van fuel benefit? page with wrong format text when input is in incorrect format" which {
          implicit lazy val result: WSResponse = {
            ...
            dropEmploymentDB()
            insertCyaData(anEmploymentUserData)
            urlPost(fullUrl(vanFuelBenefitsAmountUrl(taxYearEOY, employmentId)), welsh = user.isWelsh, headers = Seq(HeaderNames.COOKIE -> playSessionCookies(taxYear)), body = Map("amount" -> "abc"))
          }
        ...
        "should render How much was your company van fuel benefit? page with max error when input > 100,000,000,000" which {
          implicit lazy val result: WSResponse = {
            ...
            dropEmploymentDB()
            insertCyaData(anEmploymentUserData)
            urlPost(fullUrl(vanFuelBenefitsAmountUrl(taxYearEOY, employmentId)), welsh = user.isWelsh,
