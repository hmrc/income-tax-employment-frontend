...
object PageUrls extends IntegrationTest {
  ...
  def employmentEndDateUrl(taxYear: Int, employmentId: String): String = s"$appUrl/$taxYear/details/employment-end-date?employmentId=$employmentId"
  ...
  def removeExpensesUrl(taxYear: Int): String = s"$appUrl/$taxYear/remove-expenses"
  ...
  def employerOffPayrollWorkingStatusUrl(taxYear: Int): String = s"$appUrl/$taxYear/employer-off-payroll-working-warning"
