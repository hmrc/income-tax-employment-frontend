...
class EndOfYearFilterActionSpec extends UnitTest
  ...
  ".filter" should {
    ...
    "return a redirect to Income Tax Submission Overview when employmentEOYEnabled is false" in {
      val appConfig = new MockAppConfig().config(isEmploymentEOYEnabled = false)
      val underTest = EndOfYearFilterAction(taxYear = taxYearEOY, inYearUtil = inYearUtil, appConfig = appConfig)(executionContext)

      await(underTest.filter(anAuthorisationRequest)) shouldBe Some(Redirect(appConfig.incomeTaxSubmissionOverviewUrl(taxYearEOY)))
    }
