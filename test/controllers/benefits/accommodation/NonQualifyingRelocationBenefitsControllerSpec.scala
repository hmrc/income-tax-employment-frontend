...
class NonQualifyingRelocationBenefitsControllerSpec extends ControllerUnitTest
  ...
  private val pageView = inject[NonQualifyingRelocationBenefitsView]
  private val formsProvider = new AccommodationFormsProvider()

  private lazy val underTest = new NonQualifyingRelocationBenefitsController(
    mockActionsProvider,
    pageView,
  ...
  private val clazz = classOf[NonQualifyingRelocationBenefitsController]
  ...
  ".submit" should {
    ...
    "handle internal server error when accommodationService.updateNonQualifyingExpensesQuestion(...) fails" in {
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateNonQualifyingExpensesQuestion(aUser, taxYearEOY, employmentId, anEmploymentUserData, questionValue = true, Left(()))
    ...
    "save question and return correct result when question value is true" in {
      ...
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateNonQualifyingExpensesQuestion(aUser, taxYearEOY, employmentId, anEmploymentUserData, questionValue = true, Right(anEmploymentUserData))
      mockBenefitsSubmitRedirect(anEmploymentCYAModel(), NonQualifyingRelocationBenefitsAmountController.show(taxYearEOY, employmentId), taxYearEOY, employmentId, result)
    ...
    "save question and return correct result when question value is false" in {
      ...
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateNonQualifyingExpensesQuestion(aUser, taxYearEOY, employmentId, anEmploymentUserData, questionValue = false, Right(anEmploymentUserData))
