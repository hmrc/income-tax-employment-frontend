...
class QualifyingRelocationBenefitsAmountControllerSpec extends ControllerUnitTest
  ...
  private val pageView = inject[QualifyingRelocationBenefitsAmountView]
  private val formsProvider = new AccommodationFormsProvider()

  private lazy val underTest = new QualifyingRelocationBenefitsAmountController(
    mockActionsProvider,
    pageView,
  ...
  private val clazz = classOf[QualifyingRelocationBenefitsAmountController]
  ...
  ".submit" should {
    ...
    "handle internal server error when accommodationService.updateQualifyingExpenses(...) fails" in {
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateQualifyingExpenses(aUser, taxYearEOY, employmentId, anEmploymentUserData, amount = 123, Left(()))
    ...
    "save amount and perform correct redirect" in {
      ...
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateQualifyingExpenses(aUser, taxYearEOY, employmentId, anEmploymentUserData, amount = 123, Right(anEmploymentUserData))
