...
import controllers.benefits.accommodation.routes.{NonQualifyingRelocationBenefitsController, QualifyingRelocationBenefitsAmountController}
...
class QualifyingRelocationBenefitsControllerSpec extends ControllerUnitTest
  ...
  private val pageView = inject[QualifyingRelocationBenefitsView]
  private val formsProvider = new AccommodationFormsProvider()

  private lazy val underTest = new QualifyingRelocationBenefitsController(
    mockActionsProvider,
    pageView,
  ...
  private val clazz = classOf[QualifyingRelocationBenefitsController]
  ...
  ".submit" should {
    ...
    "handle internal server error when accommodationService.updateQualifyingExpensesQuestion(...) fails" in {
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateQualifyingExpensesQuestion(aUser, taxYearEOY, employmentId, anEmploymentUserData, questionValue = true, Left(()))
    ...
    "save question and return correct result when question value is true" in {
      ...
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateQualifyingExpensesQuestion(aUser, taxYearEOY, employmentId, anEmploymentUserData, questionValue = true, Right(anEmploymentUserData))
      mockBenefitsSubmitRedirect(anEmploymentCYAModel(), QualifyingRelocationBenefitsAmountController.show(taxYearEOY, employmentId), taxYearEOY, employmentId, result)
    ...
    "save question and return correct result when question value is false" in {
      ...
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateQualifyingExpensesQuestion(aUser, taxYearEOY, employmentId, anEmploymentUserData, questionValue = false, Right(anEmploymentUserData))
      mockBenefitsSubmitRedirect(anEmploymentCYAModel(), NonQualifyingRelocationBenefitsController.show(taxYearEOY, employmentId), taxYearEOY, employmentId, result)
