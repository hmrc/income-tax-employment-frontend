...
class AssetsOrAssetTransfersBenefitsControllerSpec extends ControllerUnitTest
  ...
  private val pageView = inject[AssetsOrAssetTransfersBenefitsView]
  private val formsProvider = new AssetsFormsProvider()

  private lazy val underTest = new AssetsOrAssetTransfersBenefitsController(
    mockActionsProvider,
    pageView,
    mockAssetsService,
    mockRedirectService,
  ...
  private val clazz = classOf[AssetsOrAssetTransfersBenefitsController]
  ...
  ".submit" should {
    ...
    "handle internal server error when assetsService.updateSectionQuestion(...) fails" in {
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
    ...
    "save question and return correct result when question value is true" in {
      ...
      mockBenefitsSubmitRedirect(anEmploymentCYAModel, AssetsBenefitsController.show(taxYearEOY, employmentId), taxYearEOY, employmentId, result)
