...
class CarVanFuelBenefitsControllerSpec extends ControllerUnitTest
  ...
  private val pageView = inject[CarVanFuelBenefitsView]
  private val formsProvider = new FuelFormsProvider()

  private lazy val underTest = new CarVanFuelBenefitsController(
    mockActionsProvider,
    pageView,
    mockFuelService,
    mockRedirectService,
  ...
  private val clazz = classOf[CarVanFuelBenefitsController]
  ...
  ".submit" should {
    ...
    "handle internal server error when fuelService.updateSectionQuestion(...) fails" in {
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateSectionQuestion(aUser, taxYearEOY, employmentId, anEmploymentUserData, questionValue = true, Left(()))
    ...
    "save question and return correct result when question value is true" in {
      ...
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateSectionQuestion(aUser, taxYearEOY, employmentId, anEmploymentUserData, questionValue = true, Right(anEmploymentUserData))
      mockBenefitsSubmitRedirect(anEmploymentCYAModel(), CompanyCarBenefitsController.show(taxYearEOY, employmentId), taxYearEOY, employmentId, result)
    ...
    "save question and return correct result when question value is false" in {
      ...
      mockEndOfYearSessionDataWithRedirects(taxYearEOY, employmentId, EmploymentBenefitsType, clazz = clazz, anEmploymentUserData)
      mockUpdateSectionQuestion(aUser, taxYearEOY, employmentId, anEmploymentUserData, questionValue = false, Right(anEmploymentUserData))
      mockBenefitsSubmitRedirect(anEmploymentCYAModel(), AccommodationRelocationBenefitsController.show(taxYearEOY, employmentId), taxYearEOY, employmentId, result)
