...
class AssetsOrAssetTransfersBenefitsPageSpec extends UnitTest {
  ...
  private val questionForm = new AssetsFormsProvider().assetsOrAssetTransfersForm(isAgent = user.isAgent)

  ".apply(...)" should {
    "create page model with error form when form has errors" in {
      val formWithErrors = questionForm.bind(Map(YesNoForm.yesNo -> ""))

      AssetsOrAssetTransfersBenefitsPage.apply(anyTaxYear, anyEmploymentId, user, formWithErrors, anyEmploymentUserData) shouldBe AssetsOrAssetTransfersBenefitsPage(
        taxYear = anyTaxYear,
