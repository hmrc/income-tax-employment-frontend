...
class EmployerEndDatePageSpec extends UnitTest
  ...
  ".apply(...)" should {
    ...
    "return page with pre-filled error form when form has errors" in {
      ...
      EmployerEndDatePage.apply(taxYearEOY, employmentId, aUser, formWithErrors, anEmploymentUserData) shouldBe EmployerEndDatePage(
        taxYear = taxYearEOY,
        employmentId = employmentId,
        employerName = anEmploymentUserData.employment.employmentDetails.employerName,
        isAgent = aUser.isAgent,
