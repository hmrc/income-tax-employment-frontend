...
class LivingAccommodationBenefitAmountPageSpec extends UnitTest {
  ...
  private val form = new AccommodationFormsProvider().livingAccommodationAmountForm(isAgent = user.isAgent)

  ".apply(...)" should {
    "create page model with error form when form has errors" in {
      val formWithErrors = form.bind(Map(AmountForm.amount -> ""))

      LivingAccommodationBenefitAmountPage.apply(anyTaxYear, anyEmploymentId, user, formWithErrors, anyEmploymentUserData) shouldBe LivingAccommodationBenefitAmountPage(
        taxYear = anyTaxYear,
    ...
    "create page model with empty form when accommodation amount is missing" in {
      ...
      LivingAccommodationBenefitAmountPage.apply(anyTaxYear, anyEmploymentId, user, form, employmentUserData) shouldBe LivingAccommodationBenefitAmountPage(
        taxYear = anyTaxYear,
        employmentId = anyEmploymentId,
        isAgent = user.isAgent,
        form = form
