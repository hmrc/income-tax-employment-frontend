...
class CompanyCarFuelBenefitsViewSpec extends ViewUnitTest {
  ...
  private def form(isAgent: Boolean): Form[Boolean] = new FuelFormsProvider().companyCarFuelForm(isAgent)

  private lazy val underTest = inject[CompanyCarFuelBenefitsView]
  ...
  userScenarios.foreach { userScenario =>
    ...
    s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" should {
      "render page with no pre-filled data" which {
        ...
        formPostLinkCheck(CompanyCarFuelBenefitsController.submit(taxYearEOY, employmentId).url, continueButtonFormSelector)
      ...
      "return an error page when a user submits an empty form" which {
        implicit val authRequest: AuthorisationRequest[AnyContent] = getAuthRequest(userScenario.isAgent)
        implicit val messages: Messages = getMessages(userScenario.isWelsh)
