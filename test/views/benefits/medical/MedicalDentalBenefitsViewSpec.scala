...
import controllers.benefits.medical.routes.MedicalDentalBenefitsController
...
class MedicalDentalBenefitsViewSpec extends ViewUnitTest {
  ...
  object ExpectedIndividualCY extends SpecificExpectedResults {
    ...
    val expectedErrorText = "Dewiswch ‘Iawn’ os cawsoch fuddiant meddygol neu ddeintyddol"
  ...
  object ExpectedAgentCY extends SpecificExpectedResults {
    ...
    val expectedErrorTitle = s"Gwall: $expectedTitle"
    val expectedErrorText = "Dewiswch ‘Iawn’ os cafodd eich cleient fuddiant meddygol neu ddeintyddol"
  ...
  private def form(isAgent: Boolean): Form[Boolean] = new MedicalFormsProvider().medicalDentalForm(isAgent)

  private lazy val underTest = inject[MedicalDentalBenefitsView]
  ...
  userScenarios.foreach { userScenario =>
    ...
    s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" should {
      "render page with no pre-filled data" which {
        ...
        formPostLinkCheck(MedicalDentalBenefitsController.submit(taxYearEOY, employmentId).url, continueButtonFormSelector)
