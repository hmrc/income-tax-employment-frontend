...
class OtherBenefitsViewSpec extends ViewUnitTest {
  ...
  private def form(isAgent: Boolean): Form[Boolean] = new ReimbursedFormsProvider().otherBenefitsForm(isAgent)

  private lazy val underTest = inject[OtherBenefitsView]
  ...
  userScenarios.foreach { userScenario =>
    ...
    s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" should {
      "render page with no pre-filled data" which {
        ...
        formPostLinkCheck(OtherBenefitsController.submit(taxYearEOY, employmentId).url, continueButtonFormSelector)
