  ...
  userScenarios.foreach { userScenario =>
    ...
    s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" should {
      ...
      "render page with prefilling when there is previous amount" which {
        ...
        val htmlFormat = underTest(taxYearEOY, form(userScenario.isAgent).bind(Map(AmountForm.amount -> "200")), Some(200), employmentId)
      ...
      "should render the amount page with invalid format text when input is in incorrect format" which {
        ...
        val htmlFormat = underTest(taxYearEOY, form(userScenario.isAgent).bind(Map(AmountForm.amount -> "123.33.33")), Some(200), employmentId)
