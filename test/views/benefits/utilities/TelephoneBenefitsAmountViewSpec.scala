...
class TelephoneBenefitsAmountViewSpec extends ViewUnitTest {
  ...
  private lazy val underTest = inject[TelephoneEmploymentBenefitsAmountView]
  ...
  userScenarios.foreach { userScenario =>
    ...
    s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" should {
      ...
      "should render the telephone employment benefits amount page with wrong format text when input is in incorrect format" which {
        ...
        val htmlFormat = underTest(taxYearEOY, form(userScenario.isAgent).bind(Map(AmountForm.amount -> "abc")), None, employmentId)
      ...
      "should render the telephone employment benefits amount page with max error when input > 100,000,000,000" which {
        ...
        val htmlFormat = underTest(taxYearEOY, form(userScenario.isAgent).bind(Map(AmountForm.amount -> "9999999999999999999999999999")), None, employmentId)
