  ...
  userScenarios.foreach { userScenario =>
    ...
    s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" should {
      "render page without pre-filled radio buttons" which {
        ...
        textOnPageCheck(userScenario.specificExpectedResults.get.expectedCanClaim, paragraphSelector(index = 1))
        textOnPageCheck(expectedThisIncludes, paragraphSelector(index = 2))
        ...
        textOnPageCheck(expectedFindOutMore, paragraphSelector(index = 3))
